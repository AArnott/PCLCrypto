...
public class PlatformSupport
{
    ...
    public void SymmetricEncryption(SymmetricAlgorithmName name, SymmetricAlgorithmMode mode, SymmetricAlgorithmPadding padding)
    {
        ...
        ISymmetricKeyAlgorithmProvider? algorithm = creator();
        int keyLength = algorithm.LegalKeySizes[0].MinSize;
        var keyMaterial = WinRTCrypto.CryptographicBuffer.GenerateRandom(keyLength / 8);
        using (ICryptographicKey? key = algorithm.CreateSymmetricKey(keyMaterial))
        {
    ...
    public void AsymmetricEncryption(AsymmetricAlgorithm algorithmName)
    {
        ...
        foreach (var keySize in algorithm.LegalKeySizes.SelectMany(k => k))
        {
            try
            {
                ...
                using (ICryptographicKey? key = algorithm.CreateKeyPair(keySize))
                {
                }
    ...
    {
        IKeyDerivationAlgorithmProvider? provider = WinRTCrypto.KeyDerivationAlgorithmProvider.OpenAlgorithm(KeyDerivationAlgorithm.Sp800108CtrHmacSha1);

        byte[] label = Encoding.UTF8.GetBytes("Purpose");
        byte[] context = { 1, 1, 0, 0, 0, 0, 0, 0 }; // a nonce
        IKeyDerivationParameters? parameters = WinRTCrypto.KeyDerivationParameters.BuildForSP800108(label, context);
    ...
    {
        IKeyDerivationAlgorithmProvider? provider = WinRTCrypto.KeyDerivationAlgorithmProvider.OpenAlgorithm(KeyDerivationAlgorithm.Sp80056aConcatMd5);
        ...
        IKeyDerivationParameters? parameters = WinRTCrypto.KeyDerivationParameters.BuildForSP80056a(
            algorithmId,
            partyUInfo,
            partyVInfo,
            suppPubInfo,
    ...
    {
        IMacAlgorithmProvider? provider = WinRTCrypto.MacAlgorithmProvider.OpenAlgorithm(algorithm);
        ICryptographicKey? key = provider.CreateKey(WinRTCrypto.CryptographicBuffer.GenerateRandom(32));
