...
public class SymmetricKeyAlgorithmProviderTests
{
    ...
    public void CreateSymmetricKey_InvalidInputs()
    {
        ...
        Assert.Throws<ArgumentException>(
            () => provider.CreateSymmetricKey(Array.Empty<byte>()));
        Assert.Throws<ArgumentException>(
            () =>
            {
                ICryptographicKey? key = provider.CreateSymmetricKey(new byte[4]);
    ...
    {
        ISymmetricKeyAlgorithmProvider? provider = WinRTCrypto.SymmetricKeyAlgorithmProvider.OpenAlgorithm(algorithm);
        ICryptographicKey key = provider.CreateSymmetricKey(this.keyMaterial);
        Assert.NotNull(key);
